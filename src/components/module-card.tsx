...
export function ModuleCard({ moduleId, icon, title, description }: ModuleCardProps) {
  ...
  return (
    <Card className={`transition-all duration-300 hover:shadow-lg hover:shadow-primary/20 ${isCompleted ? 'border-green-500/50 bg-card/90' : 'border-border'}`}>
        ...
        <div className="mt-4 flex items-center space-x-2">
          {loading ? (
            ...
            <>
              ...
              />
              <Label htmlFor={`module-${moduleId}`} className="flex items-center gap-2">
                {isCompleted ? (
                   <>
